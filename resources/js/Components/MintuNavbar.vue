...
<!--           <button @click="toggleNav = !toggleNav" class="p-3 opacity-75"><span v-if="!toggleNav"><icon icon="material-symbols:menu-rounded" /> </span>
        ...
        <ul :class="toggleNav ?'max-lg:block w-full':'max-lg:hidden'" @mouseover="isHovering = true" @mouseleave="isHovering = false" class="lg:flex items-center justify-start content-center">
            ...
            <li class="mx-3 my-2 text-center"><Link class="flex items-center content-center hover:bg-blue-300 rounded-lg p-1 border-b-2 hover:border-black border-indigo-400" :href="route('contact')"><span class="p-1"><icon class="text-xl opacity-80" icon="heroicons-solid:chat-bubble-left-right" /> </span>Contact</Link></li>
